...
class ExcelImporter:
    ...
    def import_excel(
        ...
        # 试运行模式：仅打印记录，不写入数据库
        if dry_run:
            logger.info("[dry_run] 文件: {} | 共 {} 条记录", excel_path, len(records))
            for idx, record in enumerate(records, start=1):
                logger.info("[dry_run] #{}: {}", idx, record)
            if errors:
                for err in errors:
                    logger.warning(
                        "[dry_run] 行 {} 处理错误: {}",
    ...
    def _build_records(
        ...
        for excel_row_num, (_, row) in enumerate(df.iterrows(), start=2):
            try:
                ...
                logger.exception("处理Excel行时发生错误，已跳过")
        ...
        def _insert_batch_rowwise(batch: list[dict]) -> tuple[int, int, int]:
            ...
            for record in batch:
                try:
                    ...
                    logger.exception("插入记录时发生错误，已跳过")
                    batch_skipped += 1
